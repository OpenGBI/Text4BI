...
def changeTemporalTrend3(timeSegmentationCondition,timeSelection):
    new_data=copy.deepcopy(TrendData11)
    timeSeg=0
    filtered_data=[]
    ...
    if timeSegmentationCondition=="30 days":
        filtered_data = [entry for entry in new_data["data"] if entry["date"] >= "2012-12-01"]
    if timeSegmentationCondition=="7 days":
        filtered_data = [entry for entry in new_data["data"] if entry["date"] >= "2012-12-23"]
